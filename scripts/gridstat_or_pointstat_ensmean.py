...
"""
Converted from scripts/gridstat_or_pointstat_ensmean.sh, this script calls the METplus
"GridStat" or "PointStat" tool to verify the ensemble mean output produced by GenEnsProd.

The script is intended to be called from jobs/GRIDSTAT_OR_POINTSTAT_ENSMEAN.sh.
"""
...
def gridstat_or_pointstat_ensmean(
    config_file: str,
    cdate: str,
    obs_dir: str,
    field_group: str,
    ...
    if geom == "grid":
        ...
        if "APCP" in met_filedir_name:
            ...
            raise ValueError(f"Invalid field group for GridStat ensmean: {field_group}")
    ...
    metplus_config_tmpl_fn = f"{metplus_tool_camel_case}_ensmean.conf"
    metplus_config_fn = (
        f"{metplus_tool_camel_case}_{met_filedir_name}_{cdate}_ensmean.conf.0"
    )
    metplus_log_fn = (
    ...
    lgr.info(f"{metplus_tool_camel_case}_ensmean completed successfully.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run METplus GridStat or PointStat on ensemble mean output"
    )
